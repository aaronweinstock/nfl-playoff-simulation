...
## tied_teams: vector of teams tied for the division lead
...
# 1. Head to head
in_division_1 = function(tied_teams, sim_df, matchups){
  pcts = c()
  for(i in tied_teams){
    match_weeks = matchups$Week[(matchups$`Winner/tie` == i & matchups$`Loser/tie` %in% tied_teams) | 
                                  (matchups$`Loser/tie` == i & matchups$`Winner/tie` %in% tied_teams)]
    match_cols = as.numeric(match_weeks) + 1
    hh = sum(sim_df[sim_df$team == i, match_cols])/length(match_cols)
    pcts = append(pcts, hh)
  }
...
# 2. Division record
in_division_2 = function(tied_teams, sim_df, matchups){
  divrec = sim_df[sim_df$team %in% tied_teams, c("team","div_pct")]
  pcts = divrec$div_pct
  still_tied = tied_teams[which(pcts == max(pcts))]
  return(still_tied)
}
...
# 3. Common games
in_division_3 = function(tied_teams, sim_df, matchups){
  same = c()
  for(i in tied_teams){
...
# 4. Conference record
in_division_4 = function(tied_teams, sim_df, matchups){
...
in_division_5 = function(tied_teams, sim_df, matchups){
  pcts = c()
  for(i in tied_teams){
...
in_division_6 = function(tied_teams, sim_df, matchups){
  pcts = c()
  for(i in tied_teams){
    team_df = matchups[matchups$`Winner/tie` == i | matchups$`Loser/tie` == i,]
...
# 7. Tiebreakers 7-11 all rely on points, which we don't predict. So move to #12, coin toss
in_division_7 = function(tied_teams){
  alive = tied_teams
...
# Function to eliminate one team at a time, per NFL tiebreaking procedures
pick_division_by_one = function(tied_teams, sim_df, matchups){
  t = length(tied_teams)
  t1 = in_division_1(tied_teams, sim_df, matchups)
  if(length(t1) == t){
    t2 = in_division_2(t1, sim_df, matchups)
  } else{
    return(t1)
  }
  if(length(t2) == t){
    t3 = in_division_3(t2, sim_df, matchups)
  } else{
    return(t2)
  }
  if(length(t3) == t){
    t4 = in_division_4(t3, sim_df, matchups)
  } else{
    return(t3)
  }
  if(length(t4) == t){
    t5 = in_division_5(t4, sim_df, matchups)
  } else{
    return(t4)
  }
  if(length(t5) == t){
    t6 = in_division_6(t5, sim_df, matchups)
  } else{
    return(t5)
  }
  if(length(t6) == t){
...
# Algorithm for breaking division ties
division_break = function(tied_teams, sim_df, matchups){
  t = 4
  comps = tied_teams
  while(t > 1){
    comps = pick_division_by_one(comps, sim_df, matchups)
    t = length(comps)
  }
  return(comps)
}
